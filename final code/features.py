...
# fixed order of feature groups (based on your example)
FEATURE_GROUP_ORDER = [
    "JCD",
    "Tamura",
    "ColorLayout",
...
def read_feature_file(path, as_flat_vector=False):
    ...
    # concatenate in a fixed order so all images have the same layout
    vecs = []
    for g in FEATURE_GROUP_ORDER:
        if g in groups:
            vecs.append(groups[g])
        else:
            # if some group is missing, just skip, or you could pad with zeros
            pass
